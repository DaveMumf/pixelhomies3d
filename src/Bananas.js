...
function Banana({ index, z, speed }) {
  ...
  return (
    <Detailed ref={ref} distances={[0, 0, 0]}>
     <group scale={[0.08, 0.13, 0.08]}>
...
export default function Bananas({ speed = 10, count = 80, depth = 80, easing = (x) => Math.sqrt(1 - Math.pow(x - 1, 2)) }) {
  return (
    // No need for antialias (faster), dpr clamps the resolution to 1.5 (also faster than full resolution)
    <Canvas gl={{ antialias: false }} dpr={[1, 1.5]} camera={{ position: [0, 0, 10], fov: 10, near: 0.01, far: depth + 15 }}>
      <color attach="background" args={['#ffffff']} />
      <spotLight position={[10, 20, 10]} penumbra={1} intensity={0.5} color="white" />
      ...
      <EffectComposer multisampling={0}>
        <DepthOfField target={[0, 0, 60]} focalLength={0.8} bokehScale={8} height={700} />
      </EffectComposer>
    </Canvas>
  )
}
